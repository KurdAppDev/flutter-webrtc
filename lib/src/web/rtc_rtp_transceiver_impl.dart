...
extension RTCRtpTransceiverInitWebExt on RTCRtpTransceiverInit {
  dynamic toJsObject() => jsutil.jsify({
        'direction': typeRtpTransceiverDirectionToString[direction],
        if (streams != null) 'streams': streams!.map((e) => (e as MediaStreamWeb).jsStream).toList(),
        if (sendEncodings != null) 'sendEncodings': sendEncodings!.map((e) => e.toMap()).toList(),
      });
}
...
class RTCRtpTransceiverWeb extends RTCRtpTransceiver {
  ...
  @override
  Future<TransceiverDirection?> getCurrentDirection() async => typeStringToRtpTransceiverDirection[jsutil.getProperty(_jsTransceiver, 'currentDirection')];

  @override
  Future<TransceiverDirection> getDirection() async => typeStringToRtpTransceiverDirection[jsutil.getProperty(_jsTransceiver, 'direction')]!;
